...
int MAIN(MAIN_FUNCTION_PARAMETERS)
{
  ...
  initilizationSmartHome();
  InitRelayPins();
#endif
  // Run the application  zz, which usually never terminates.
  loop();
...
static void loop(void)
{
  ...
  //char messageRX[]="chinmay surjith 1 2";
    while (true) {
...
/*
    uint32_t sensorData=0;
        ...
        {
        	emberAfGuaranteedPrintln("Error in measurement in main",0);
        }
        else
        {
